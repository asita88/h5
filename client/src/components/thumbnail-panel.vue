<template>
  <div class="page-thumbnail-panel" v-loading="loading" v-if="pageData">
    ...
    <div class="page-item-title border-T ellipsis">
      <span class="item-title-i" :title="pageData.title">{{
        pageData.title || "未命名作品"
      }}</span>
    </div>
    <div class="border-T thumbnail-panel-btn" v-if="btnList.length">
      ...
      <div class="btn-wrapper" v-if="btnList.includes('useTemplate')">
        <el-button type="text" size="mini" @click="copyPage"
          >使用模板</el-button
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    newPage() {
      ...
      this.$API
        ...
        .then((res) => {
          this.loading = false;
          if (res.body) {
            this.$router.push({ name: "Editor", query: { id: res.body.id } });
          }
        })
        .catch(() => {
          this.loading = false;
    ...
    // 编辑
    edit() {
      this.$router.push({ name: "Editor", query: { id: this.pageData.id } });
    },
    // 复制页面
    copyPage() {
      this.loading = true;
      this.$API
        .copyPage({ id: this.pageData.id })
        .then((res) => {
          this.loading = false;
          this.$router.push({ name: "Editor", query: { id: res.body.id } });
        })
    ...
    publish() {
      this.loading = true;
      this.$API
        .publishPage({ id: this.pageData.id })
        .then(() => {
          this.loading = false;
    ...
    setTemplate() {
      this.loading = true;
      this.$API
        .setTemplatePage({ id: this.pageData.id })
        .then(() => {
          this.loading = false;
    ...
    delete() {
      this.$alert("确认删除页面？删除后，将无法访问此页面?", "操作提示", {
        ...
        this.$API.deletePage({ id: this.pageData.id }).then(() => {
