module.exports = (app) => ({
  ...
  async updateUserPass() {
    ...
    // 校验老密码是否正确
    const userCurrentPass = await $service.user.getUsersPasswordByUsername(
      userData.username
    );
    const verifyPass = await $helper.checkPassword(
      oldPass,
      userCurrentPass.password
    );
    if (!verifyPass) {
      $helper.returnBody(false, "", "原密码错误，请输入正确原密码！");
      return;
    }
    const pass = $helper.createPassword(newPass);
  ...
  async updateUserAvatar() {
    ...
    let fileResult = await $service.file.upload(
      file,
